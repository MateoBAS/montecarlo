...
BrownianBridge::BrownianBridge(int steps, double T) : numSteps(steps), totalT(T) {
    ...
    // El primer Z de Sobol decide el punto final (vencimiento)
    bridgeIdx[0] = steps;
    leftIdx[0] = 0;
    rightIdx[0] = 0;
    ...
    // Algoritmo de bisección para rellenar los puntos medios
...
void BrownianBridge::transformToStandardNormals(const std::vector<double>& sobolZ, std::vector<double>& impliedZ) const {
    ...
    // 1. Asignar el vencimiento
    W[bridgeIdx[0]] = stdDev[0] * sobolZ[0];
    
    // 2. Interpolar el resto de la trayectoria
    ...
    // 3. Convertir el camino (W) de vuelta a Normales Estándar (Z') 
    // para que tu paso de Euler siga funcionando sin cambios.
