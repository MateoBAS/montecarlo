...
double GBMAsset::simulateFinalValue(double totalTime, int numSteps,
                                    const Eigen::Ref<const Eigen::RowVectorXd>& z_shocks,
                                    const std::vector<double>& ratePath) const {
    return simulateGbmTerminal(initialPrice, driftSpread, volatility, totalTime, numSteps,
                               z_shocks, ratePath);
}
...
std::vector<double> GBMAsset::generatePath(double totalTime, int numSteps,
                                           const Eigen::Ref<const Eigen::RowVectorXd>& z_shocks,
                                           const std::vector<double>& ratePath) const {
    return simulateGbmPathWithRate(initialPrice, driftSpread, volatility, totalTime, numSteps,
                                   z_shocks, ratePath);
}
